import { IDice, IRound } from '@/interfaces';
import { useReducer, useState } from 'react';
import { calculateFinalScore, rollDice } from '@/utils';
import { Button, Dice, RoundCounter, Row } from '@/components';
import { sheetReducer } from '@/reducers';
import { sheetInitialValue } from '@/reducers/Sheet/helpers';

interface SheetProps {
  initialDice: IDice;
}

export const Sheet = ({ initialDice }: SheetProps) => {
  const [isEyeOn, setIsEyeOn] = useState(false);
  const [dice, setDice] = useState(initialDice);
  const [sheet, dispatchSheet] = useReducer(sheetReducer, sheetInitialValue);
  const [rounds] = useState<IRound[]>(
  ...
  return (
    <div>
      <div className="flex flex-row justify-between items-center py-2">
        ...
        <Button onClick={() => setIsEyeOn(!isEyeOn)} size="sm">
          <svg
            viewBox={isEyeOn ? '0 0 576 512' : '0 0 512 512'}
            width={50}
            height={39}
      ...
      <Row {...{ dispatchSheet, sheet, isEyeOn, dice }} id="row1" color="red" />
      <Row
        {...{ dispatchSheet, sheet, isEyeOn, dice }}
        id="row2"
        color="yellow"
      />
      <Row
